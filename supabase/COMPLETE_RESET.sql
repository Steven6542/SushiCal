...
-- 执行此脚本将：
-- 1. 删除所有现有表
-- 2. 重新创建全新的表结构 (包含多地区价格、图片字段)
-- 3. 设置所有必要的安全策略 (RLS) 和管理员权限
-- 4. 导入预置品牌数据
...
CREATE TABLE public.brand_plates (
  ...
  image_url TEXT DEFAULT NULL, -- 新增: 图片字段
  regional_prices JSONB DEFAULT NULL, -- 新增: 多地区价格
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
BEGIN
  ...
  VALUES (
    ...
    false -- 默认为false，但初始种子数据或后续脚本可将其设为true
  );
  RETURN new;
END;
...
-- Brands (Allow Admins Full Access)
...
CREATE POLICY "Users can manage own brands" ON public.brands FOR ALL TO authenticated USING (auth.uid() = user_id OR (SELECT is_admin FROM public.user_profiles WHERE id = auth.uid()) = true);

-- Brand Plates (Allow Admins Full Access)
...
CREATE POLICY "Users manage own brand_plates" ON public.brand_plates FOR ALL TO authenticated USING (
  EXISTS (SELECT 1 FROM public.brands WHERE brands.id = brand_plates.brand_id AND (brands.user_id = auth.uid() OR (SELECT is_admin FROM public.user_profiles WHERE id = auth.uid()) = true))
);

-- Brand Side Dishes (Allow Admins Full Access)
...
CREATE POLICY "Users manage own brand_side_dishes" ON public.brand_side_dishes FOR ALL TO authenticated USING (
  EXISTS (SELECT 1 FROM public.brands WHERE brands.id = brand_side_dishes.brand_id AND (brands.user_id = auth.uid() OR (SELECT is_admin FROM public.user_profiles WHERE id = auth.uid()) = true))
);
...
-- Storage Permissions (Admin Full Access)
...
CREATE POLICY "Public read access for brand logos" ON storage.objects FOR SELECT USING (bucket_id = 'brand-logos');

DROP POLICY IF EXISTS "Public read access for menu items" ON storage.objects;
CREATE POLICY "Public read access for menu items" ON storage.objects FOR SELECT USING (bucket_id = 'menu-items');

DROP POLICY IF EXISTS "Authenticated users can upload brand logos" ON storage.objects;
CREATE POLICY "Authenticated users can upload brand logos" ON storage.objects FOR INSERT WITH CHECK (bucket_id IN ('brand-logos', 'menu-items') AND auth.role() = 'authenticated');
